...
uint8_t currentVideoMode = 0x03;

uint8_t registers_80x25_text[] =
{
...
void writeVideoRegisters(uint8_t* regs)
{
	unsigned i;

	/* write MISCELLANEOUS reg */
	outputByteToPort(VGA_MISC_WRITE, *regs);
	regs++;
	/* write SEQUENCER regs */
	for (i = 0; i < VGA_NUM_SEQ_REGS; i++)
	{
		outputByteToPort(VGA_SEQ_INDEX, i);
		outputByteToPort(VGA_SEQ_DATA, *regs);
		regs++;
	}
	/* Unlock CRTC registers */
	outputByteToPort(VGA_CRTC_INDEX, 0x03);
	outputByteToPort(VGA_CRTC_DATA, inputByteFromPort(VGA_CRTC_DATA) | 0x80);
	outputByteToPort(VGA_CRTC_INDEX, 0x11);
	outputByteToPort(VGA_CRTC_DATA, inputByteFromPort(VGA_CRTC_DATA) & ~0x80);
	...
	{
		outputByteToPort(VGA_CRTC_INDEX, i);
		outputByteToPort(VGA_CRTC_DATA, *regs);
		regs++;
	}
	/* write GRAPHICS CONTROLLER regs */
	for (i = 0; i < VGA_NUM_GC_REGS; i++)
	{
		outputByteToPort(VGA_GC_INDEX, i);
		outputByteToPort(VGA_GC_DATA, *regs);
		regs++;
	}
	/* write ATTRIBUTE CONTROLLER regs */
	for (i = 0; i < VGA_NUM_AC_REGS; i++)
	{
		(void)inputByteFromPort(VGA_INSTAT_READ);
		outputByteToPort(VGA_AC_INDEX, i);
		outputByteToPort(VGA_AC_WRITE, *regs);
		regs++;
	}
	/* lock 16-color palette and unblank display */
	(void)inputByteFromPort(VGA_INSTAT_READ);
	outputByteToPort(VGA_AC_INDEX, 0x20);
}
...
void setPlane(uint8_t plane)
{
	...
	planeMask = 1 << plane;
	// Set read plane 
	outputByteToPort(VGA_GC_INDEX, 4);
	outputByteToPort(VGA_GC_DATA, plane);
	// Set write plane 
	outputByteToPort(VGA_SEQ_INDEX, 2);
	outputByteToPort(VGA_SEQ_DATA, planeMask);
}
...
uint8_t* getFrameBufferBase()
{
	...
	outputByteToPort(VGA_GC_INDEX, 6);
	plane = inputByteFromPort(VGA_GC_DATA);
	plane >>= 2;
	plane &= 3;
	switch (plane)
	{
	case 0:
	case 1:
		base = (uint8_t*)P2V(0xA0000);
		break;

	case 2:
		base = (uint8_t*)P2V(0xB0000);
		break;

	case 3:
		base = (uint8_t*)P2V(0xB8000);
		break;
	}
	return base;
}
...
void writeFont(uint8_t * fontBuffer, unsigned int fontHeight)
{
	...
	// Save VGA registers
	outputByteToPort(VGA_SEQ_INDEX, 2);
	seq2 = inputByteFromPort(VGA_SEQ_DATA);

	outputByteToPort(VGA_SEQ_INDEX, 4);
	seq4 = inputByteFromPort(VGA_SEQ_DATA);
	outputByteToPort(VGA_SEQ_DATA, seq4 | 0x04);

	outputByteToPort(VGA_GC_INDEX, 4);
	gc4 = inputByteFromPort(VGA_GC_DATA);

	outputByteToPort(VGA_GC_INDEX, 5);
	gc5 = inputByteFromPort(VGA_GC_DATA);
	outputByteToPort(VGA_GC_DATA, gc5 & ~0x10);

	outputByteToPort(VGA_GC_INDEX, 6);
	gc6 = inputByteFromPort(VGA_GC_DATA);
	outputByteToPort(VGA_GC_DATA, gc6 & ~0x02);
	
	setPlane(2);
	// Write font to video memory
	fontBase = (uint8_t*)P2V(0xB8000); 
	for (i = 0; i < 256; i++)
	{
		memmove((uint16_t*)fontBase, fontBuffer, fontHeight);
		fontBase += 32;
		fontBuffer += fontHeight;
	}
	// Restore VGA registers 
	outputByteToPort(VGA_SEQ_INDEX, 2);
	outputByteToPort(VGA_SEQ_DATA, seq2);
	outputByteToPort(VGA_SEQ_INDEX, 4);
	outputByteToPort(VGA_SEQ_DATA, seq4);
	outputByteToPort(VGA_GC_INDEX, 4);
	outputByteToPort(VGA_GC_DATA, gc4);
	outputByteToPort(VGA_GC_INDEX, 5);
	outputByteToPort(VGA_GC_DATA, gc5);
	outputByteToPort(VGA_GC_INDEX, 6);
	outputByteToPort(VGA_GC_DATA, gc6);
}
